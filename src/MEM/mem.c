...
static int (*fp)(void) = (int (*)(void))NULL;   /* out-of-memory handler */
static int mem_count;         /* # of allocs that haven't been free'd   */
static int mem_scount;        /* # of sallocs that haven't been free'd */
static int  mem_exception(void); /* called when out of memory */
static void  mem_fillin(char *fil,int lin);
...
static void mem_fillin(fil,lin)
char *fil;
int lin;
{
...
int lin;
{
      ...
      if(ptr == NULL )
      {     fprintf(ferr,"mem_free: freeing NULL pointer; ");
            goto err;
      }
      if (mem_count <= 0)
      {     fprintf(ferr,"mem_free: more frees than allocs; ");
            goto err;
      }
      dl = mem_ptrtodl(ptr);
      if (dl->beforeval != BEFOREVAL)
      {
	    /* If this is overwritten, it's likely that the dl
	     * structure is also garbage.  Don't try to use it. -IAN!
	     */
            fprintf(ferr,"mem_free: pointer x%lx unknown or underrun\n",
		(long)ptr);
            goto err;
      }
...
#endif
      {
            fprintf(ferr,"mem_free: pointer x%lx overrun\n",(long)ptr);
            goto err2;
      }
