...
int main(void){
  ...
  //
  //Because of the direction of the electric field we only expect sites:
  //0,1,1
  //1,1,1
  //
  //And 
  ...
  ArbArray ArClLL = ClusterSort( TotalOrders, ArClLL2);

  FilterCluster( TotalOrders, OrderL, MasterM, &ArClLL, snA, PeriodicX, PeriodicY, PeriodicZ, XElecOn, YElecOn, ZElecOn);

	CalculateNeighNodes(TotalOrders, &ArClLL, snA, PeriodicX, PeriodicY, PeriodicZ);
  ...
  //printArbArray(ArClLL);
	printf("Testing:OccAllCluster\n");
	//printClusterLL((ClusterLL)getClusterList(getSN(snA,3,3,3)));
