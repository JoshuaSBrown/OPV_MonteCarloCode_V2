...
/* Function grabs the cluster ID given the SiteNode ID and 
 * Site Node array
 */
int getClusterIDGivenSiteNodeID(SNarray snA, int SiteID);

/*Function grabs the cluster ID given the SiteNode
 */
int getClusterIDGivenSiteNode(SiteNode sn);

/* Similar to FilterCluster but only looks at one Cluster and does not delete anything
 * Only conducts the part that actually determines how nodes within a cluster are 
 * connected to each other
 */
int DetermineNodeOrientationSingleCluster(ClusterLL * TempClLL ,\
                                          const_SNarray snA    );

/* This joins two ClusterLL together
 */
int mergeClusterLLGivenSiteNodeIDs( ClusterLL ClLLAll, SNarray snA,\
                          matrix MasterM, int SiteID1, int SiteID2,\
                          ParameterFrame PF);

/* Checks to see if site nodes are attached to the 
 * same cluster. If they are returns a 1 else
 * it returns a 0.
 */
