...
int main(void){
	...
	ClusterLL ClLL3 = newClusterLL(3);
	rv = addNodesToClusterGivenSites(ClLL3, Site5, Site6);
	assert(rv==0);
	rv = DetermineNodeOrientationSingleCluster(&ClLL3,snA);
	assert(rv==0);
	rv = CalculateNeighNodesForSingleClusterLL(ClLL3,snA,PeriodicX,PeriodicY, PeriodicZ);
	assert(rv==0);
	rv = CalculateSumAndPGivenSingleClusterLL(snA, ClLL3, MasterM,attempts, PeriodicX, PeriodicY, PeriodicZ);
	assert(rv==0);
	tempSN =  getSNwithInd(snA,Site5);
	setDataStruc(&tempSN,1,(void *)ClLL3);
	tempSN =  getSNwithInd(snA,Site6);
	setDataStruc(&tempSN,1,(void *)ClLL3);

	rv = appendClusterLL(ClLLAll,ClLL3);
	assert(rv==0);
	rv = mergeClusterLLGivenSiteNodeIDs( ClLLAll, 
			snA,
			MasterM,
