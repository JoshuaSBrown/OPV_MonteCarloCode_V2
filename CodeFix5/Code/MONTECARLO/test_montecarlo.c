...
int main() {
  ...
  double ratio = 0.001;
	...
	rv = CorrCal(NULL, i, j, k, Rad, SiteEnergy, \
					 ...
					 PeriodicX, PeriodicY, PeriodicZ, lambda, ratio);
	assert(rv==-1);
	...
	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor,&distanceij,  3,\
					 PeriodicX, PeriodicY, PeriodicZ, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij, SeedProt,\
					 -1, PeriodicY, PeriodicZ, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij, SeedProt,\
					 2, PeriodicY, PeriodicZ, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij, SeedProt,\
					 PeriodicX, -1, PeriodicZ, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij,SeedProt,\
					 PeriodicX, 2, PeriodicZ, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij, SeedProt,\
					 PeriodicX, PeriodicY, -1, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij, SeedProt,\
					 PeriodicX, PeriodicY, 2, lambda,ratio);
	assert(rv==-1);

	rv = CorrCal(seed, i, j, k, Rad, SiteEnergy, \
					 SiteDistance, snA, &SumCor, &SumEcor, &distanceij, SeedProt,\
					 PeriodicX, PeriodicY, PeriodicZ, 0,ratio);
	assert(rv==-1);
