		...
		for(j = 0; j < getAwid(snA); j++){
			for(k = 0; k < getAhei(snA); k++){
				...
				for(l=0;l<6;l++){
					...
					setE(MasterM,getIndex(snA,i,j,k)+1,6+(l+1), v[l]);
				}
			}
		}
	}
...
int CalculateNeighNodesForClusterLL(ClusterLL TempClLL, SNarray snA, int PeriodicX, int PeriodicY, int PeriodicZ ){
  ...
  printf("CalculateNeighNodesForClusterLL\n");
  if(PeriodicX==1 && PeriodicY==1 && PeriodicZ==1){	
    CalculateNeighPXPYPZ(TempClLL, snA);
  }else if(PeriodicX == 0 && PeriodicY==1 && PeriodicZ==1){
    CalculateNeighPYPZ(TempClLL, snA);
  }else if(PeriodicX == 1 && PeriodicY==0 && PeriodicZ==1){
    CalculateNeighPXPZ(TempClLL, snA);
  }else if(PeriodicX == 1 && PeriodicY==1 && PeriodicZ==0){
    CalculateNeighPXPY(TempClLL, snA);
  }else if(PeriodicX == 0 && PeriodicY==0 && PeriodicZ==1){
    CalculateNeighPZ(TempClLL, snA);
  }else if(PeriodicX == 1 && PeriodicY==0 && PeriodicZ==0){
    CalculateNeighPX(TempClLL, snA);
  }else if(PeriodicX == 0 && PeriodicY==1 && PeriodicZ==0){
    CalculateNeighPY(TempClLL, snA);
  }else{
    CalculateNeigh(TempClLL, snA);
  }
...
int CalculateNeighNodes(int TotalOrders,  ArbArray * ClArLL, SNarray snA, int PeriodicX, int PeriodicY, int PeriodicZ ){
	...
	for(element=0;element<TotalOrders;element++){
		TempClLL=(ClusterLL) getArbElement((*ClArLL),element);
    CalculateNeighNodesForClusterLL(TempClLL, snA, PeriodicX, PeriodicY,PeriodicZ );
  }
...
int DetermineClusterStatus(SiteNode sn1, SiteNode sn2){
  ...
  if(sn1==NULL){
    #ifdef _ERROR_
    fprintf(stderr,"ERROR In DetermineClusterStatus sn1 is NULL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
  ...
  if(sn2==NULL){
    #ifdef _ERROR_
    fprintf(stderr,"ERROR in DetermineClusterStatus sn2 is NULL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
  ...
  flag = 3;
  count = 0;
  if(checkSNconnectedCluster(sn1)==1){
    count++;
    flag = 1;
  }
  if(checkSNconnectedCluster(sn2)==1){
    count++;
    flag = 2;
  }
  
  if(count==2){
    //Both sites are part of a different cluster
    return 0;
  }else if(count==1){
    //One of the sites is part of a cluster the other isn't.
    if(flag==1){
      //Site 1 is part of a cluster
      return 1;
    }else{
      //Site 2 is part of a cluster
      //Check that they are not part of the same cluster
      if(getCluster_id((ClusterLL)getClusterList(sn1))==getCluster_id((ClusterLL)getClusterList(sn2))){
        return 4;
      }else{
        return 2;
      }
...
int CalculateSumAndPGivenSingleClusterLL(const_SNarray snA, ClusterLL TempClLL,\
	...
	if(PeriodicX<0){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR PeriodicX is less than 0 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(PeriodicY<0){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR PeriodicY is less than 0 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(PeriodicZ<0){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR PeriodicZ is less than 0 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(PeriodicX>1){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR PeriodicX is greater than 1 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(PeriodicY>1){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR PeriodicY is greater than 1 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(PeriodicZ>1){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR PeriodicZ is greater than 1 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(snA==NULL){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR snA is NULL in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(TempClLL ==NULL){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR ClArLL is NULL in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(MasterM==NULL){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR MasterM is NULL in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	if(attempts<1){
		#ifdef _ERROR_
    fprintf(stderr,"ERROR attempts are less than 1 in CalculateSumAndPGivenSingleClusterLL\n");
    #endif
    #ifdef _FORCE_HARD_CRASH_
    exit(1);
    #endif
	...
	//Cylce through the nodes in tempClLL and for every node
	//determine the hop rate off of the cluster thet total
	//average of all the hop rates off the cluster will be
	//used to calculate the dwell time for a charge stuck 
	//in the cluster.
  ...
  //neighbor site. 
  tempNode = getStartNode(TempClLL);
  printf("ClusterFunctionsStep50\n");
  mtxProbNeighDwell = CalculateProbNeighDwell(countNeighOpts,mtxDwellTime,mtxProb,\
      tempNode,MasterM,snA, rateN,\
      PeriodicX, PeriodicY, PeriodicZ);

  printf("ClusterFunctionsStep51\n");
  //Now we calculate the pval for the cluster
  //pval - Stay in Cluster
  //pval - Hop off cluster
  //pval - sites off cluster
...
matrix CalculateProbNeighDwell(const int countNeighOpts,\
	...
	while(tempNode!=NULL){
		...
		Node_ID = getNode_id(tempNode);
		Check = checkBoundsIndexFront(snA, Node_ID);
		...
		Check = checkBoundsIndexBehind(snA, Node_ID);
		if(Check==1 || PeriodicX!=0){
			if(getFlagBeh(tempNode)!=1){
		...
		Check = checkBoundsIndexLeft(snA, Node_ID);
		...
		Check = checkBoundsIndexRight(snA, Node_ID);
		if(Check==1 || PeriodicY!=0){
			if(getFlagRig(tempNode)!=1){
		...
		Check = checkBoundsIndexBelow(snA, Node_ID);
		...
		Check = checkBoundsIndexAbove(snA, Node_ID);
		if(Check==1 || PeriodicZ!=0){
			if(getFlagAbo(tempNode)!=1){
...
matrix CalculateDwellTimeAndRateN(ClusterLL *  TempClLL, matrix * mtxTimes  ,\
       const_matrix mtxProbNeigh, const_matrix MasterM , const_SNarray snA  ,\
...
int FindCluster( int * OrderL, SNarray snA, double electricEnergyX,\
  ...
  printf("Cluster Sort\n");
	...
	ArbArray ClArLL = ClusterSort( TotalOrders, ArLL);
	...
	CalculateNeighNodes(TotalOrders, &ClArLL, snA,\
			PeriodicX, PeriodicY, PeriodicZ );
	ArbArrayCheck(ClArLL);
...
int LoadCluster( char * FileName, int * OrderL, SNarray * snA, double *electricEnergyX,\
  ...
  DIR * dir = opendir("CLUSTERFILE");
  if(dir){
    closedir(dir);
  }else {
    fprintf(stderr,"ERROR Unable to open directory for some reason\n");
    return -1;
  }
	...
	if(stat("CLUSTERFILE",&st)==-1){
        ...
        Hop h2;
				...
				*ClArLL1 = newArbArray(totalElem,1);
                ...
                setNeighLL_addNeighNode(NeiLL,NeighNod2);
                NeighNod = getNextNeigh(NeighNod);
							}
  ...
  while (TempClLL!=NULL){
    ...
    while (tempNode!=NULL){
      ...
      if(getFlagRig(tempNode)==0){
        //Neighbor that is to the right
        Node_IDRig=getIndRigP(snA, i, j, k);
        printf("NodeIDRigh %d",Node_IDRig);
        //If ID is -1 means it's outside the boundaries
         addNeighNodeToCluster(&TempClLL, Node_IDRig);
        if(Elec_Rig==-1) {
          setCluster_elecYid(&TempClLL,1);
